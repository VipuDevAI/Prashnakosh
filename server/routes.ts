  ...
  app.post("/api/blueprints/:id/generate-preview", requireAuth, requireTenant, requireRole("hod", "admin", "super_admin"), async (req: Request, res) => {
    try {
      ...
      const selectionResult = await storage.selectQuestionsUnified(blueprint, {
        ...
        departmentId: blueprint.departmentId || undefined,
        setCount: setCount || 1,
      ...
      if (existing.blueprintId && (!existing.questionIds || existing.questionIds.length === 0)) {
        ...
        if (blueprint && blueprint.sections) {
          ...
          const selectionResult = await storage.selectQuestionsUnified(blueprint, {
            ...
            departmentId: blueprint.departmentId || existing.departmentId || undefined,
            setCount: 1,
            fixedOrder: true,
    ...
    try {
      ...
      if (schoolLogoUrl) {
        try {
          ...
          doc.image(logoBuffer, doc.page.width / 2 - 40, doc.y, { width: 80, height: 80 });
          doc.moveDown(5);
        } catch (logoErr) {
